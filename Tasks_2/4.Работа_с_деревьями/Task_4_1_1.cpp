...
int main()
{
    ...
    else if (!cur->parent) cout << "Нет племянников (эл-т - корень)\n";
    else {
        tree *p = cur->parent, *b;
        if (p->left == cur) b = p->right;
        else b = p->left;
        if (!b) cout << "Нет племянников (нет 'брата')\n";
        else {
            bool fl = false;
            if (b->left) {
                if (!fl) cout << "Племянники: ";
                cout << b->left->data << ' ';
                fl = true;
            }
            if (b->right) {
